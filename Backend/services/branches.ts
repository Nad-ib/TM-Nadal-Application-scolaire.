...
> => {
    ...
    return Object.keys(grouped).map((key) => {
        ...
        return {
            ...
            avg: Math.round((grouped[monthIdx].sum / grouped[monthIdx].count) * 10) / 10,
        };
    });
};
...
export const fetchCustomGroupsWithData = async (userId: string) => {
    ...
    return await Promise.all(
        groups.map(async (group) => {
            ...
            const totalPoints = validAverages.reduce((acc, b) => acc + b.moyenne_globale, 0);
            const average = validAverages.length > 0 ? totalPoints / validAverages.length : 0;
