...
export default function IconPicker({ onSelect, onClose }: IconPickerProps) {
	...
	useEffect(() => {
		const fetchIcons = async () => {
			const query = search.trim() || "education"; //à la place éducation mettre le nom de la branche
			setLoading(true);

			try {
				const res = await fetch(
					`https://api.iconify.design/search?query=${query}&limit=32`,
				);
						...
						<div className="grid grid-cols-4 gap-3">
							{icons.map((iconName) => (
								<button
									...
									<Icon
										icon={iconName}
										className="text-3xl text-gray-600 group-hover:text-blue-600"
									/>
								</button>
							))}
						</div>
