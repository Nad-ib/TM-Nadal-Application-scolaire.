...
export default function NoteItem({
	...
	const CardContent = (
		<div
			className={`w-full p-4 bg-white rounded-2xl shadow-sm border flex flex-col gap-3 transition-colors ${is_group ? "hover:bg-gray-50 border-blue-100" : "border-gray-100"}`}>
			...
			<div className="flex items-center gap-4 text-sm text-gray-500">
				...
				<div className="w-px h-3 bg-gray-200" />
				<span>{is_group ? "Série" : `Date: ${date}`}</span>
			</div>
		</div>
	);
